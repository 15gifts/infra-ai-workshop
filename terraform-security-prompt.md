...
```
<html>
  <head>  <!-- @import fonts, inline <style>, inline <script> for sort -->
  <body>

    <!-- Sticky nav bar -->
    <nav class="sticky-bar">
      CRITICAL: N  |  HIGH: N  |  Services affected: N  |  Files scanned: N
    ...
    <section class="findings-summary">
      ...
      <!-- Each ID is an anchor link to the finding card in the service section below -->
      <!-- AVD column: "AVD ↗" as a small external link, opens in new tab -->
      <!-- Default sort: Severity (CRITICAL first), then Service alphabetically -->
      <!-- Inline <script> enables click-to-sort on all column headers -->

    <!-- One section per affected AWS service, ordered by highest severity first,
         then by number of findings descending -->
    <section class="service-block" id="svc-s3">
    <section class="service-block" id="svc-rds">
    <section class="service-block" id="svc-iam">
    <!-- ... one per service ... -->

      <!-- Each service section contains: -->

      <div class="service-header">
        <!-- Service name (e.g. "S3") and AWS service icon/label -->
        <!-- Severity bar: red blocks for CRITICAL count + orange blocks for HIGH count -->
        <!-- e.g.  ████ CRITICAL: 3   ██ HIGH: 2  -->
        <!-- Total finding count badge -->

      <!-- Finding cards within this service, CRITICAL first then HIGH,
           sorted by file path then line number within each severity tier -->

      <div class="finding-card severity-{critical|high}" id="TF-001">
        <!-- Severity badge | ID | Rule name -->
        <!-- Resource: <terraform resource label> (monospace pill) -->
        <!-- File: <path>  Line: <N> (monospace pill) -->
        <!-- Description paragraph -->
        <!-- Recommendation (blue-tinted callout box) -->
        <!-- AVD Reference: <avd-url> — clickable link, opens in new tab -->
...
#### Deriving the service name from a finding

Map the Terraform resource type to a display service name using this table:

| Resource type prefix | Display name |
|---|---|
| `aws_s3_` | S3 |
| `aws_db_`, `aws_rds_` | RDS |
| `aws_iam_` | IAM |
| `aws_lambda_` | Lambda |
| `aws_security_group`, `aws_vpc`, `aws_subnet`, `aws_flow_log` | VPC / Network |
| `aws_ecr_` | ECR |
...
| `aws_lb_`, `aws_alb_` | ALB |
| `aws_cloudfront_` | CloudFront |
| `aws_msk_` | MSK |
| `terraform` (backend block) | Terraform State |
| anything else | Other |

Group findings from the same service type together regardless of which resource instance triggered the finding.

#### Visual details

- **Service header**: dark card (`#1e293b` background), service name in large bold text, severity bar rendered as a row of coloured square blocks — red (`#ef4444`) for each CRITICAL finding, orange (`#f97316`) for each HIGH finding, with counts labelled alongside
- **Finding cards**: 4px left border in severity colour; no top/bottom margin between cards within the same service section (they stack flush to read as a continuous list)
...
- AVD reference link: small muted text below the recommendation, `↗` icon, opens in new tab
- Summary table: zebra-striped rows, sticky header, click-to-sort via inline `<script>` (no external libraries)
